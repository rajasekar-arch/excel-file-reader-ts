 ...
 */
export async function getExcelRowCount(filePath: string, sheetName?: string): Promise<number> {
 ...
 */
export async function getExcelHeaders(filePath: string, sheetName?: string): Promise<string[]> {
  if (!fs.existsSync(filePath)) {
    throw new Error(`File not found at: ${filePath}`);
  }
 ...
 */
export async function isExcelColumnPopulated(filePath: string, headerName: string, sheetName?: string): Promise<boolean> {
  if (!fs.existsSync(filePath)) {
    throw new Error(`File not found at: ${filePath}`);
  }
